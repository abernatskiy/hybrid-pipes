import { type BlockRef, PortalAbstractStream, type StreamOptions } from '@abernatskiy/hybrid-pipes-core'
...
import { NetworksMappings } from './constants/networks'
import { handleAerodromeBasicSwap } from './handlers/handle-aerodrome-basic-swap'
import { handleAerodromeSlipstreamSwap } from './handlers/handle-aerodrome-slipstream-swap'
import { handleUniswapV2Swap } from './handlers/handle-uniswap-v2-swap'
import { handleUniswapV3Swap } from './handlers/handle-uniswap-v3-swap'
import { type PoolMetadata, PoolMetadataStorage } from './pool-metadata-storage'
          ...
          const events = blocks
            .flatMap((block: any) => {
              ...
              return block.logs.map((log) => {
                const transaction = block.transactions.find((tx) => tx.hash === log.transactionHash)
                if (!transaction) {
                  this.logger.error(
