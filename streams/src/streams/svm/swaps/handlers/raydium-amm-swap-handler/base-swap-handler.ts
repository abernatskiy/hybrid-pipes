...
export abstract class RaydiumCpmmSwapBaseHandler {
  ...
  protected getTokenData(): { inputToken: Token; outputToken: Token } {
    ...
    const inputVaultTokenBalance = tokenBalances.find((tb) => tb.account === inputVault)
    const outputVaultTokenBalance = tokenBalances.find((tb) => tb.account === outputVault)
  ...
  protected getTokenReserves({
    ...
    const { preAmount: preSwapInputTokenReserves, postDecimals: inputTokenDecimals } =
      inputVaultTokenBalance

    const { preAmount: preSwapOutputTokenReserves, postDecimals: outputTokenDecimals } =
