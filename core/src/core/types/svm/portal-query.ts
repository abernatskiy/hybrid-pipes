import type { Base58, Base64 } from '../primitives'
import type { Select, Selector, Simplify, Trues } from '../utils'
...
export type BlockHeader<F extends BlockHeaderFieldSelection = Trues<BlockHeaderFieldSelection>> =
  Select<BlockHeaderFields, F>

export type TransactionFieldSelection = Selector<keyof TransactionFields>
export type Transaction<F extends TransactionFieldSelection = Trues<TransactionFieldSelection>> =
  Select<TransactionFields, F>

export type InstructionFieldSelection = Selector<keyof InstructionFields>
export type Instruction<F extends InstructionFieldSelection = Trues<InstructionFieldSelection>> =
  Select<InstructionFields, F>

export type LogMessageFieldSelection = Selector<keyof LogMessageFields>
export type LogMessage<F extends LogMessageFieldSelection = Trues<LogMessageFieldSelection>> =
  Select<LogMessageFields, F>

export type BalanceFieldSelection = Selector<keyof BalanceFields>
export type Balance<F extends BalanceFieldSelection = Trues<BalanceFieldSelection>> = Select<
  BalanceFields,
  F
>

export type TokenBalanceFieldSelection = Selector<keyof TokenBalanceFields>
export type TokenBalance<F extends TokenBalanceFieldSelection = Trues<TokenBalanceFieldSelection>> =
  Select<{ [K in keyof TokenBalanceFields]: TokenBalanceFields[K] }, F>

export type RewardFieldSelection = Selector<keyof RewardFields>
export type Reward<F extends RewardFieldSelection = Trues<RewardFieldSelection>> = Select<
  RewardFields,
  F
>
...
export type BlockData<F extends FieldSelection> = {
  // Number and hash are always present in the response
  // regardless being requested or not
