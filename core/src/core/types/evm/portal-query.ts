...
import type { ConditionalOmit, Select, Selector, Simplify, Trues } from '../utils'

type AddPrefix<Prefix extends string, S> = S extends string ? `${Prefix}${Capitalize<S>}` : never

type RemovePrefix<Prefix extends string, T> = T extends `${Prefix}${infer S}`
  ? Uncapitalize<S>
  : never
...
export type BlockHeader<T extends BlockHeaderFieldSelection = Trues<BlockHeaderFieldSelection>> =
  Select<BlockHeaderFields, T>

export type TransactionFieldSelection = Selector<keyof TransactionFields>
export type Transaction<T extends TransactionFieldSelection = Trues<TransactionFieldSelection>> =
...
export type TraceFieldSelection = Simplify<
  TraceCreateFieldSelection &
    TraceCallFieldSelection &
    TraceSuicideFieldSelection &
    TraceRewardFieldSelection
>
...
export type TraceCreateResult<
  F extends TraceCreateFieldSelection = Trues<TraceCreateFieldSelection>,
> = Select<TraceCreateResultFields, RemoveKeysPrefix<'createResult', F>>

export type TraceCallAction<F extends TraceCallFieldSelection = Trues<TraceCallFieldSelection>> =
  Select<TraceCallActionFields, RemoveKeysPrefix<'call', F>>

export type TraceCallResult<F extends TraceCallFieldSelection = Trues<TraceCallFieldSelection>> =
  Select<TraceCallResultFields, RemoveKeysPrefix<'callResult', F>>

export type TraceSuicideAction<
  F extends TraceSuicideFieldSelection = Trues<TraceSuicideFieldSelection>,
> = Select<TraceSuicideActionFields, RemoveKeysPrefix<'suicide', F>>

export type TraceRewardAction<
  F extends TraceRewardFieldSelection = Trues<TraceRewardFieldSelection>,
> = Select<TraceRewardActionFields, RemoveKeysPrefix<'reward', F>>

export type TraceCreate<F extends TraceCreateFieldSelection = Trues<TraceCreateFieldSelection>> =
  Simplify<
...
export type TraceCall<F extends TraceCallFieldSelection = Trues<TraceCallFieldSelection>> =
  Simplify<
    Select<TraceCallFields, F> &
      ConditionalOmit<
        { action: TraceCallAction<F>; result?: TraceCallResult<F> },
        { [k: string]: never } | undefined
      >
  >

export type TraceSuicide<F extends TraceSuicideFieldSelection = Trues<TraceSuicideFieldSelection>> =
  Simplify<
    Select<TraceSuicideFields, F> &
      ConditionalOmit<{ action: TraceSuicideAction<F> }, { [k: string]: never } | undefined>
  >

export type TraceReward<F extends TraceRewardFieldSelection = Trues<TraceRewardFieldSelection>> =
  Simplify<
    Select<TraceRewardFields, F> &
      ConditionalOmit<{ action: TraceRewardAction<F> }, { [k: string]: never } | undefined>
  >
...
export type StateDiffNoChange<F extends StateDiffFieldSelection = Trues<StateDiffFieldSelection>> =
  Select<StateDiffNoChangeFields, F>

export type StateDiffAdd<F extends StateDiffFieldSelection = Trues<StateDiffFieldSelection>> =
  Select<StateDiffAddFields, F>

export type StateDiffChange<F extends StateDiffFieldSelection = Trues<StateDiffFieldSelection>> =
  Select<StateDiffChangeFields, F>

export type StateDiffDelete<F extends StateDiffFieldSelection = Trues<StateDiffFieldSelection>> =
  Select<StateDiffDeleteFields, F>

export type StateDiff<F extends StateDiffFieldSelection = Trues<StateDiffFieldSelection>> =
  F extends any
...
export type BlockData<F extends FieldSelection> = {
  header: Pick<BlockHeaderFields, 'number' | 'hash'> & BlockHeader<F['block'] & {}>
  logs?: Log<F['log'] & {}>[]
