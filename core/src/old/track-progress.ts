import type { Offset } from '../old/portal-abstract-stream'
...
export type Stats = { bytesReceived: 0 }
export type TrackProgressTick = { offset: Offset; stats: Stats; ts: number }
...
export class TrackProgress {
  ...
  track(offset: Offset, stats: Stats) {
    if (!this.initial) {
    ...
    this.interval = setInterval(async () => {
      ...
      const bytesDownloaded = this.last ? this.current.stats.bytesReceived - this.last.stats.bytesReceived : 0
